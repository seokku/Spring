...
// java 코드에서 jvava.util.Date 객체를 사용하여야 한다.
// java 코드에서 java.sql.Date 객체 사용하게 되면 날짜만 처리하게 된다. 시간이 없어진다.
function dateToDateStr(date, sep){
	
	// 날짜를 찍어서 확인해서 숫자는 나오는데 ~~~000시간 정보가 포함되어 있지 않다.
	// oracle DB - to_char(writeDate, 'hh:mi:ss') from board_reply 확인해 본다.
...
// 날짜 데이터를 timestemp라는 Long 타입의 긴 숫자를 받아서 날짜계산에 의해 현재시간 기준으로
// 24가 지났으면 문자열을 , 그렇지 않으면 숫자 문자열을 돌려주는 함수.
function displyTime(timeStemp){
//	alert(timeStemp);
	// 오늘 날짜 객체 만들기
	var today = new Date();
	// 오늘 날짜 타임스템프에서 비교할 날짜의 타임스템프를 빼기
	var gap = today.getTime() - timeStemp;
//	alert(gap);
	
	// 24시간이 지나지 않았으면 시간을 지났으면 날짜를 문자열로 돌려준다.
...
function ajaxPage(pageObject) {
	...
	for (i = pageObject.startPage; i <= pageObject.endPage; i++) {
		...
		if (pageObject.page == i) str += "<a href='' onclick='return false'>" + i + "</a>";
			...
			+ " data-toggle='tooltip' data-placement='top'  onclick='return false'>" + i + "</a>";
		str += "</li>";
	}
