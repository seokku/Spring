...
public class FileUtil {
	...
	public static File noDuplicate(String fileFullName) throws Exception{
		...
		System.out.print("FileUtil.noDuplicate().fileFullName" + fileFullName);
		
		File file = null;
		int dotPos = fileFullName.lastIndexOf(".");
		// image.jpg 의 fileName은 image, ext : .jpg
		...
		// 파일 정보 확인
		System.out.print("FileUtil.noDuplicate().fileName = " + fileName + ", ext =" + ext);
		
		while(true) {
			if(cnt ==0)
				file = toFile(fileFullName);
			else file = toFile(fileName + cnt + ext);
			// 중복이 되면 계속 반복처리 한다. 중복이 되지 않으면 빠져 나간다.
			if(!exist(file)) break;
			// 중복이 된다.
			cnt++;
